...
"нониллион\0дециллион\0одна\0две\0?\0";
const int pos_ru[7][10] = {
    ...
    {828, 812, 821, 828, 828, 828, 828, 828, 828, 828}
};
    ...
    for (int i = 0; value[i]; ++i) {
        ...
        if (!rank--) {
            if (zeros != 3) {
                ...
                if (!exp);
                else if (1 < num && num < 5) buffadd((exp == 1) ? " и" : " а");
                else if (exp == 1 && num == 1) buffadd(" а");
                else if (exp > 1 && num >= 5) buffadd(" ов");
            }
            zeros = 0;
            rank = 2;
            --exp;
